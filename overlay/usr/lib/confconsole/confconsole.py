...
class TurnkeyConsole:
    ...
    def usage(self) -> str:
        ...
        ip6_addr = self._get_ip6addr(ifname) if ifname else None
        ip6_display = ip6_addr if ip6_addr else "not configured"

        text = Template(t).safe_substitute(ipaddr=ip_addr, ip6addr=ip6_display)
